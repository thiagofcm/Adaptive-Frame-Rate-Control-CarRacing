...
class AdaptiveFPS_Random_Initial_Pos(gym.Wrapper):
    """Same as AdaptiveFPS_TrackAware_Wrapper, except the car spawns at a random point
    around the track each episode instead of always the start line. Intended to be
    used with env_id="CarRacing_VarFramerate_RandomStart" (envs/car_racing_var_fps_
    random_start.py) -- see reset() for what happens if paired with a plain
    CarRacing_VarFramerate env_id instead."""
    ...
    def reset(self, *, seed=None, options=None):
        # Intended to be used with env_id="CarRacing_VarFramerate_RandomStart"
        # (envs/car_racing_var_fps_random_start.py), which spawns the car at a random
        # track node each episode and exposes that choice as env.unwrapped.spawn_idx.
        # If paired with the plain "CarRacing_VarFramerate" env_id by mistake, this
        # falls back to spawn_idx=0 below (getattr default) -- same fixed-start
        # behavior as AdaptiveFPS_TrackAware_Wrapper, not a crash.
        ...
        # spawn_idx: which track node the car actually spawned at this episode (see
        # CarRacing_VarFramerate_RandomStart.reset()). Used below in the RAW/unrolled
        # index space for CautiousVars seeding, and to reindex this wrapper's own
        # local track_xy copy so everything downstream measures from the spawn point.
        spawn_idx = int(getattr(self.env.unwrapped, "spawn_idx", 0))

        # Goal Settings: fixed absolute arc-length distance from the SPAWN point (not
        # a fraction of track point count -- see AdaptiveFPS_TrackAware_Wrapper.__init__
        # for why goal_distance is an absolute distance, not a percentage). Rolling
        # track_xy here is a lossless cyclic relabeling of the closed loop (total
        # length, and every adjacent-pair distance, are unchanged) -- so track_xy[0]
        # below becomes the spawn point and every line after this one is unmodified
        # from the fixed-start version.
        track_xy = np.roll(track[:, 2:4].astype(np.float64), -spawn_idx, axis=0)
        ...
        # the spawn point (track_xy[0] post-roll -- see CarRacing_VarFramerate_RandomStart
        ...
        # CautiousVars builds its OWN xy/heading/curv arrays straight from
        # env.unwrapped.track in its NATURAL (unrolled) order -- a separate array from
        # this wrapper's locally-rolled track_xy above. Seed its nearest-node search
        # window at the RAW spawn_idx (not the rolled index space) so the very first
        # get_cautious_var() call below doesn't search around node 0 while the car is
        # actually spawned elsewhere -- last_idx is a plain public attribute, not
        # private state, so this is standard "seed the search window" usage, not a hack.
        self.cautious_sensors.last_idx = spawn_idx
