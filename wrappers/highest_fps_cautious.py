...
class HighestFPS_Cautious_Wrapper(gym.Wrapper):
    ...
    def step(self, fps_action):
        ...
        self.cautious_obs = self.last_sampled_cautious_obs
        #print(f"nav: {(b-a)*1000:.2f}  envstep(skip+render): {(c-b)*1000:.2f}  cautious: {(d-c)*1000:.2f}")
        ...
        #print(f"[Highest FPS Cautious Wrapper] Step: {self.world_step_count}, Obs: {debug_obs}, Action: {self.current_fps}")

        #print(f"[Highest FPS Cautious Wrapper] Step: {self.world_step_count}, Fresh Obs: {frame_consumed}, Obs: {self.cautious_obs}, Action: {self.current_fps} FPS")
        ...
        return self.cautious_obs, reward, terminated, truncated, info