...
class AdaptiveFPS_Cautious_SL_Wrapper_Ep(gym.Wrapper):
    def __init__(self, env, nav_model_path, device, frame_cost=0.0, budget=50, gamma=0.99, max_physics_steps=1000):
        super().__init__(env)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.current_obs = self._get_augmented_obs(self.last_sampled_cautious_obs, ticks_in_window=0)
        return self.current_obs, info

    def _get_augmented_obs(self, obs, ticks_in_window):
        cautious_obs = obs
        # obs_age_ratio now reflects how long the *just-completed* decision window was,
        # since the FPS policy is only ever queried on fresh, non-stale observations
    ...
    def step(self, fps_action):
        ...
        # The action IS this window's decision — apply it immediately
        ...
        for _ in range(self.obs_interval):
            ...
            # Nav controller always acts on the last *sampled* frame — held fixed for the whole window
            ...
            #     f"Obs Interval: {self.obs_interval}, Episode Frame Count: {self.episode_frame_count}, OBS: {self.current_obs}")
        ...
        # Frame cost — one window consumed, so one frame_cost charge, applied once per decision
        frame_penalty = self.frame_cost
        total_reward -= frame_penalty
        self.fps_penalty += frame_penalty

        # Budget penalty — same one-shot logic as before, now evaluated once per decision
        ...
        self.current_obs = self._get_augmented_obs(self.last_sampled_cautious_obs, ticks_in_window)
