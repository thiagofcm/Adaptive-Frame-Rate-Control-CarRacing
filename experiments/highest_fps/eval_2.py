...
def make_eval_env(env_id, seed, nav_model_path, device) :
    ...
    env = HighestFPSWrapper(env, nav_model_path, device)
    env.reset(seed=seed)
    return env
...
def main():
    ...
    p.add_argument("--out_dir", default="experiments/highest_fps/eval")
    ...
    nav_model_path = NAV_MODEL_PATH

    returns, lengths = [], []
    for ep in range(args.episodes):
        env = make_eval_env(args.env_id, args.seed + ep, nav_model_path, device)
        ...
        while not done:
            ...
            if env.world_step_count % obs_interval == 0:
                last_sampled_obs = obs_t
                frame_consumed = True
                obs_type = 'Fresh'
                total_frame_counter+=1
            else:
                print(f" Step: {env.world_step_count} - Stale Observation - X: {x}, Y: {y}")
            ...
            print(f" Step: {env.world_step_count} - {obs_type} Observation - Frames Consumed: {total_frame_counter} -  X: {x}, Y: {y}, Action:{action}")
        ...
        plot_track_trajectory(
            ...
            save_path=os.path.join(args.out_dir, f"traj_seed{args.seed}_fps{args.fps}_ret{int(ep_return)}_goal.png"),
        ...
        if args.save_video and frames:
            ...
            video_dir = os.path.join(args.out_dir, run_string,ckpt_string)
