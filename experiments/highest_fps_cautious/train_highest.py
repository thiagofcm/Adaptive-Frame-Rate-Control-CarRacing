...
def make_env(env_id, idx,nav_model_path, capture_video, run_name, device):
    def thunk():
        # remove capture_video
        ...
        env = HighestFPS_Cautious_Wrapper(env, nav_model_path, device)
        return env
    return thunk
...
if __name__ == "__main__":
    ...
    run_name = f"{args.env_id}_ppo_highest_cautious_{date_str}"
    ...
    writer = SummaryWriter(f"experiments/highest_fps_cautious/runs/{run_name}")
    writer.add_text(
        "hyperparameters",
        "|param|value|\n|-|-|\n%s" % ("\n".join([f"|{key}|{value}|" for key, value in vars(args).items()])),
    )

    info_file = os.path.join(f"experiments/highest_fps_cautious/runs/{run_name}", "info_settings.txt")
    ...
    envs = gym.vector.SyncVectorEnv(
        [make_env(args.env_id, i, nav_model_path, args.capture_video, run_name, device) for i in range(args.num_envs)],
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if iteration % 50 == 0 or iteration == args.num_iterations:
            ckpt_path = f"experiments/highest_fps_cautious/runs/{run_name}/agent_step{global_step}.pt"
            torch.save({
                "iteration": iteration,
    ...
    torch.save({
        ...
        "agent_state_dict": agent.state_dict(),
        "args": vars(args),
    }, f"experiments/highest_fps_cautious/runs/{run_name}/final.pt")
    envs.close()
    writer.close()