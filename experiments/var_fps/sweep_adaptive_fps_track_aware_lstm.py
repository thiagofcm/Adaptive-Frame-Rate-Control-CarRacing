#!/usr/bin/env python3
"""Sweep runner for the adaptive-FPS track-aware (LSTM) experiment.

Reads a sweep config (default: config/sweep_adaptive_fps_track_aware_lstm.yaml)
containing every base hyperparameter plus a `sweep:` section mapping parameter
names to lists of values. Builds the Cartesian product of the swept parameters,
materializes one full config file per combination, and launches
train_adaptive_fps_track_aware_lstm.py (repo root) once per combination -- each
pinned to its own isolated CPU core (via taskset) and round-robined across
available GPUs.
"""
import argparse
import itertools
...
# SCRIPT_DIR is experiments/var_fps/ -- two levels below the actual repo root (the
# directory containing wrappers/, envs/, config/, etc.), not one.
REPO_ROOT = os.path.abspath(os.path.join(SCRIPT_DIR, "..", ".."))
TRAIN_SCRIPT = os.path.join(SCRIPT_DIR, "train_adaptive_fps_track_aware_lstm.py")
DEFAULT_SWEEP_CONFIG = os.path.join(REPO_ROOT, "config", "sweep_adaptive_fps_track_aware_lstm.yaml")
...
def parse_args():
    ...
    p.add_argument("--extra", type=str, default="",
                    help="extra CLI args forwarded verbatim to train_adaptive_fps_track_aware_lstm.py")
    return p.parse_args()
...
def detect_gpus():
    ...
    # and NavModel does batch-1 CNN inference once per physics tick, where GPU kernel-
...
def main():
    ...
    print("[sweep] compare with: tensorboard --logdir runs/adaptive_fps_track_aware_lstm")
