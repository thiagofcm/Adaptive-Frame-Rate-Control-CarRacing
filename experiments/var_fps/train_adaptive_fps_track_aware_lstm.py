...
class Args:
    ...
    """add potential-based reward shaping (PBRS) on top of the existing reward pipeline
    -- see AdaptiveFPS_TrackAware_Wrapper._potential(). Off by default, matching the
    wrapper's own default, so existing configs are unaffected unless explicitly opted in."""
    shaping_gamma: float = 0.99
    """discount used inside the PBRS shaping term itself -- should match --gamma (the
    PPO discount) for the shaping to be theoretically reward-neutral in expectation;
    only used when --use-potential-shaping is set"""
    potential_lambda: float = 1.0
    """scales the PBRS shaping term before it's added to the reward; only used when
    --use-potential-shaping is set"""
...
def make_env(env_id, nav_model_path, frame_cost, budget, max_episode_steps, device,
             use_potential_shaping=False, shaping_gamma=0.99, potential_lambda=1.0):
    def thunk():
...
if __name__ == "__main__":
    ...
    env_fns = [make_env(args.env_id, nav_model_path, args.frame_cost, args.budget, args.max_episode_steps, device,
                         args.use_potential_shaping, args.shaping_gamma, args.potential_lambda)
    ...
    for iteration in range(start_iteration, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if args.use_potential_shaping:
                writer.add_scalar("charts/potential_lambda", args.potential_lambda, global_step)
