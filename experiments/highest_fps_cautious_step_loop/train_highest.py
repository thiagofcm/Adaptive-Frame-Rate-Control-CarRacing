...
from wrappers.highest_fps_cautious_step_loop  import HighestFPS_Cautious_SL_Wrapper
...
class Args:
    ...
    # Highest FPS Experiment arguments
    nav_model_path: str = None
    """the path to the navigation model"""
...
def make_env(env_id,nav_model_path, device):
    def thunk():
        ...
        env = HighestFPS_Cautious_SL_Wrapper(env, nav_model_path, device)
        return env
    return thunk
...
def load_args(args_class):
    ...
    # 3. let tyro parse remaining CLI flags, using YAML values as defaults
...
if __name__ == "__main__":
    ...
    run_name = f"{args.env_id}_ppo_highest_cautious_sl_{date_str}"
    ...
    writer = SummaryWriter(f"experiments/highest_fps_cautious_step_loop/runs/{run_name}")
    writer.add_text(
        "hyperparameters",
        "|param|value|\n|-|-|\n%s" % ("\n".join([f"|{key}|{value}|" for key, value in vars(args).items()])),
    )

    info_file = os.path.join(f"experiments/highest_fps_cautious_step_loop/runs/{run_name}", "info_settings.txt")
    ...
    envs = gym.vector.SyncVectorEnv(
        [make_env(args.env_id, nav_model_path, device) for i in range(args.num_envs)],
    )
    ...
    start_iteration = 1
    if args.resume_path is not None:
        train_ckpt = torch.load(args.resume_path, map_location=device)
        agent.load_state_dict(train_ckpt["agent_state_dict"])
        optimizer.load_state_dict(train_ckpt["optimizer_state_dict"])
        global_step = train_ckpt["global_step"]
        start_iteration = train_ckpt["iteration"] + 1
        print(f"Resumed from checkpoint: {args.resume_path} (iteration {train_ckpt['iteration']}, step {global_step})")
    else:
        global_step = 0
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            #print(f"policy: {(t1-t0)*1000:.1f}ms  env.step: {(t2-t1)*1000:.1f}ms")
            ...
            if "episode" in infos:
                finished = infos.get("_episode", np.zeros(args.num_envs, dtype=bool))
                for i in range(args.num_envs):
                    if finished[i]:
                        ...
                        writer.add_scalar("charts/episodic_length", int(ep_length), global_step)
        ...
        if iteration % 50 == 0 or iteration == args.num_iterations:
            ckpt_path = f"experiments/highest_fps_cautious_step_loop/runs/{run_name}/agent_step{global_step}.pt"
            torch.save({
                "iteration": iteration,
    ...
    #torch.save(agent.state_dict(), f"runs/{run_name}/final.pt")
    torch.save({
        "iteration": args.num_iterations,
        "global_step": global_step,
        "agent_state_dict": agent.state_dict(),
        "args": vars(args),
    }, f"experiments/highest_fps_cautious_step_loop/runs/{run_name}/final.pt")
    envs.close()
    writer.close()