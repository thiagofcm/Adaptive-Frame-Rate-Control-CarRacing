...
from wrappers.adaptive_fps_cautious_sl_wrapper import AdaptiveFPS_Cautious_SL_Wrapper_Ep
...
def make_env(env_id, nav_model_path, frame_cost, budget, max_episode_steps, device):
    def thunk():
        ...
        env = AdaptiveFPS_Cautious_SL_Wrapper_Ep(
            env, nav_model_path, device, frame_cost, budget,
            gamma=args.gamma, max_physics_steps=max_episode_steps,   # ← new
        )
        return env
    return thunk
...
if __name__ == "__main__":
    ...
    run_folder = "experiments/adaptive_fps_step_loop/runs"
    os.makedirs(run_folder, exist_ok=True)
    run_name = f"adaptive_fps_sl_fc_{args.frame_cost}_bud_{args.budget}_{date_str}"
    ...
    info_file = os.path.join(f"{run_folder}/{run_name}", "info_settings.txt")
    ...
    for iteration in range(start_iteration, args.num_iterations + 1):
        ...
        if iteration % 5 == 0:  # save every 50 iterations
            checkpoint_path = f"{run_folder}/{run_name}/ckpts/timestep_{global_step}_iterations_{iteration}"
    ...
    torch.save({
        ...
        "args": vars(args),
        "global_step": global_step,
    }, f"{run_folder}/{run_name}/model.pt")
    print(f"Model saved → {run_folder}/{run_name}/model.pt")
    writer.close()