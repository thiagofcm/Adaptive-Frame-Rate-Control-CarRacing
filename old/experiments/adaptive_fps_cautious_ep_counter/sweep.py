#!/usr/bin/env python3
"""Sweep runner for the adaptive-FPS cautious (ep_counter) experiment.

Reads a sweep config (default: config/sweep_adaptive_fps_cautious_ep_counter.yaml)
...
train_cautious_lstm_ep_counter.py once per combination -- each pinned to its own
...
TRAIN_SCRIPT = os.path.join(SCRIPT_DIR, "train_cautious_lstm_ep_counter.py")
DEFAULT_SWEEP_CONFIG = os.path.join(REPO_ROOT, "config", "sweep_adaptive_fps_cautious_ep_counter.yaml")
...
def parse_args():
    ...
    p.add_argument("--extra", type=str, default="",
                    help="extra CLI args forwarded verbatim to train_cautious_lstm_ep_counter.py")
    return p.parse_args()
...
def main():
    ...
    print("[sweep] compare with: tensorboard --logdir experiments/adaptive_fps_cautious_ep_counter/runs")
