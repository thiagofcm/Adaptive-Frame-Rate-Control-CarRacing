...
Reads a sweep config (default: config/sweep_adaptive_fps_smdp_mlp.yaml)
...
train_mlp.py once per combination -- each pinned to its own isolated CPU core
(via taskset) and round-robined across available GPUs.
"""
import argparse
import itertools
import os
...
TRAIN_SCRIPT = os.path.join(SCRIPT_DIR, "train_mlp.py")
...
def parse_args():
    ...
    p.add_argument("--extra", type=str, default="",
                    help="extra CLI args forwarded verbatim to train_mlp.py")
    return p.parse_args()
...
def main():
    ...
    print("[sweep] compare with: tensorboard --logdir smdp/experiments/adaptive_fps_smdp/runs")
